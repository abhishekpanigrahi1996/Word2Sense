...
from representations.matrix_serializer import save_matrix, save_vocabulary, load_count_vocabulary, load_matrix
...
def calc_pmi(counts, cds, alpha=1.0):
    """
    Calculates e^PMI; PMI without the log().
    """
    #sum_w = []
    #for i in range(counts.get_shape()[0]):
    #    sum_w.append( np.sum(counts.getrow(i).data ))

    #sum_c = []
    #for i in range(counts.get_shape()[1]):
    #    sum_c.append( np.sum(counts.getcol(i).data ))

    #sum_w = np.asarray(sum_w)
    #sum_c = sum_w#np.asarray(sum_c) 
    ...
    pmi = pmi * sum_total #* (sum_w ** (alpha - 1.0))
    #(sum_total ** (alpha + cds - 1.0))
    return pmi
