...
def read_vocab(corpus_file, thr):
    ...
    #find the top 50000 elements
    #count_list = np.asarray([count for token, count in vocab.items()])
    #thr = np.argsort(count_list)[50000]
    #import pickle
    #list_topk = pickle.load(open('reqd_wts.pkl', 'rb'))
    #return dict([(token, count) for token, count in vocab.items() if count >= thr and len(token)>1 and token in list_topk])
